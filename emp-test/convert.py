...
def splitline(txt_convert, line_split, indent):
    ...
    while i < len(line_split):
        temp = line_split[i]
        # if temp.find('declareinbranch') != -1:
        #     i += 1
        #     continue
...
def convert(src_dir = 'src'):
    ...
    for file in file_names:
        ...
        txt_convert += 'cout<< "checksum:\t"<< checksum.reveal<int32_t>()<<endl;\n'
                
        txt_convert_final = ""
        txt_convert_final += '#include "emp-sh2pc/emp-sh2pc.h"\n'
        txt_convert_final += 'using namespace emp;\n'
        txt_convert_final += 'using namespace std;\n'
        txt_convert_final += 'int main(int argc, char** argv) {\n'
        txt_convert_final += '    int port, party;\n'
        txt_convert_final += '    parse_party_and_port(argv, &party, &port);\n'
        txt_convert_final += '    NetIO * io = new NetIO(party==ALICE ? nullptr : "127.0.0.1", port);\n'
        txt_convert_final += '    setup_semi_honest(io, party);\n'
